...
# -*- coding: utf-8 -*-
"""
check_database_status_simple.py

Simple diagnostic script to check what data exists in the Neo4j database
without requiring APOC plugin.
"""
...
def check_database_status():
    ...
    print("Database Status Check (Simple)")
    print("=" * 40)
    ...
    try:
        with driver.session(database=cfg["database"]) as session:
            ...
            print(f"  ✓ Temporal Relationships: {'YES' if has_temporal_rels else 'NO'}")
            ...
            # Sample filings
            result = session.run("MATCH (f:Filing) RETURN f.formType, f.filedAt, f.accession LIMIT 3")
            filings = list(result)
            if filings:
                print("  Recent Filings:")
                for record in filings:
                    print(f"    - {record['f.formType']} ({record['f.filedAt']}) - {record['f.accession']}")
            else:
            ...
            if has_10k_data and not has_news_data:
                print("  ✅ PARTIAL: You have 10-K data but no news data")
                print("     → Run: python simple_evaluation.py (works with 10-K only)")
                print("     → OR add news data: python ingest_news_to_neo4j.py")
            elif has_10k_data and has_news_data and has_sentiment_rels and has_news_org_rels:
            ...
            if has_10k_data and not has_news_data:
                print("  For immediate evaluation (10-K only):")
